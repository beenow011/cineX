...
export const MovieInput = () => {
    ...
    const { mutate: searchMovie } = trpc.retriveMoviesFromImdb.useMutation({
        onSuccess: (data) => {
            console.log(data)
        }
    }
    ...
    return (
        <div className="w-full h-full mx-2.5 bg-slate-700 p-3 divide-y-2 md:divide-y-0 flex-col md:flex-row md:divide-x-2 divide-slate-400 flex mt-3.5 justify-between items-center rounded-md">
            <div className="p-3 flex-1 flex flex-col justify-center items-center">
                <h1 className="text-red-600 mb-3">Search the movie by IMDB id</h1>
                ...
                <Button className="my-3" onClick={() => searchMovie({ imdbId })}> Search </Button>
            </div>

            <div className="p-3 flex-1 flex flex-col justify-center items-center">
                <h1 className="text-red-600">Search the movie by name</h1>
